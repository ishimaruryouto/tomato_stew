...
export default function OtherPost() {
    ...
    // 今回は Yamamoto 固定（後で UI で切替するなら state にする）
    const locationName = "Yamamoto";

    useEffect(() => {
        setLoading(true);
        setErrText("");

        // locationName は "Yamamoto" で完全一致
        // createdAt で並べたいので orderBy を付ける
        // ここは composite index が必要になることがある（where + orderBy）
        const q = query(
            collection(db, "posts"),
            ...
            (snap) => {
                const list: PhotoData[] = snap.docs.map((d) => {
                    ...
                    return {
                        id: d.id,
                        src: data.imageUrl,
                        station: data.locationName ?? "Unknown",
        ...
        <>
            {modalData && (
                ...
                >
                    <div className="relative" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="mt-4 px-15.5">
                            <p className="font-bold">コメント</p>
                            <p className="mt-2 leading-relaxed">{modalData.comment || "（コメントなし）"}</p>
                        </div>
            ...
            <div className="pt-6">
                ...
                <div className="relative mt-19 drop-shadow-[0_2px_4px_rgba(34,34,34,0.30)]">
                    ...
                    <div className="absolute top-0 left-11 w-78.75 h-103.75 bg-[#F9F9F9] z-20 grid grid-cols-2 gap-5 place-items-center ml-2">
                        {photos.length === 0 ? (
                            <p className="col-span-2 text-gray-500 text-sm">{emptyText}</p>
                        ) : (
                            photos.map((p) => (
                                <div
                                    key={p.id}
                                    className="w-28 h-44.5 bg-[#FEFEFE] rounded-bl-[4px] rounded-br-[4px] drop-shadow-[0_2px_4px_rgba(34,34,34,0.30)] cursor-pointer relative"
                                    onClick={() => setModalData(p)}
                                >
